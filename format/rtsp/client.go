...
import (
	...
	"github.com/wargebitebane/video/codec"
	"github.com/wargebitebane/video/codec/aacparser"
	"github.com/wargebitebane/video/codec/h264parser"
	"github.com/wargebitebane/video/format/rtsp/sdp"
	"github.com/wargebitebane/video/utils/bits/pio"
	"io"
	"log"
	"net"
	"net/textproto"
...
func (self *Client) probe() (err error) {
	for {
		...
		if _, err = self.readPacket(); err != nil {
			log.Println("__________________________________________READ PACKET ERR", err)
			return
		}
	}
	self.stage = stageCodecDataDone
	log.Println("__________________________________________RETURN???")
	return
}

func (self *Client) prepare(stage int) (err error) {
	for self.stage < stage {
		log.Println("__________________________________________Stage is!", self.stage)
		switch self.stage {
		case 0:
			log.Println("__________________________________________DDDDDDDDDDDD", self.stage)
			if err = self.Options(); err != nil {
				log.Println("__________________________________________Options ERR!", err)
				return
			}
		case stageOptionsDone:
			if _, err = self.Describe(); err != nil {
				log.Println("__________________________________________Describe ERR!", err)
				return
			}
		case stageDescribeDone:
			if err = self.SetupAll(); err != nil {
				log.Println("__________________________________________SetupAll ERR!", err)
				return
			}

		case stageSetupDone:
			if err = self.Play(); err != nil {
				log.Println("__________________________________________Play ERR!", err)
				return
			}

		case stageWaitCodecData:
			if err = self.probe(); err != nil {
				log.Println("__________________________________________PROBE ERR!", err)
				return
			}
		}
	}
...
func (self *Client) Streams() (streams []av.CodecData, err error) {
	if err = self.prepare(stageCodecDataDone); err != nil {
		log.Println("_______________________________________________STREAM ERR", err)
		return
	}
	for _, si := range self.setupIdx {
		stream := self.streams[si]
...
func (self *Stream) makeCodecData() (err error) {
	...
	if media.PayloadType >= 96 && media.PayloadType <= 127 {
		log.Println("MEDIATYPE", media.Type, self.Sdp)
		switch media.Type {
		case av.H264:
		...
		case av.OPUS:
			self.CodecData = codec.NewOPUSCodecData(48000)
		default:
			log.Fatalln("Fix Format may be raw PCM 97", media.PayloadType, media.Type)
		}
	} else {
		log.Println("MEDIATYPE next", media.Type, self.Sdp, media.PayloadType)
...
func (self *Stream) handleH263Payload(timestamp uint32, packet []byte) (err error) {
	if len(packet) < 2 {
...
func (self *Stream) handleRtpPacket(packet []byte) (err error) {
	...
	*/
	...
	// log.Println("SDP TYPE", self.Sdp.Type)
	switch self.Sdp.Type {
	case av.MP4V_ES:
		if err = self.handleH263Payload(timestamp, payload); err != nil {
			return
		}
	case av.H264:
